...
void main() async {
  ...
  var content = cp949.decode(bytes);
  
  // Use regex to split by any common line ending
  var lines = content.split(RegExp(r'\r\n|\r|\n'));
  
  // Clean up empty trailing line if it exists
  if (lines.isNotEmpty && lines.last.isEmpty) {
    lines.removeLast();
  }

  print('Original line count: ${lines.length}');

  // Remove existing redundant entries
  lines.removeWhere((line) => 
    line.startsWith('"901"') || line.startsWith('"902"') || 
    line.startsWith('"903"') || line.startsWith('"904"') || 
    line.startsWith('"905"') || line.startsWith('"906"') || 
    line.startsWith('"907"') || line.startsWith('"908"')
  );

  print('Cleaned line count: ${lines.length}');
  ...
  // Join with \r\n to match common Windows CSV format if that's what's used
  final finalContent = lines.join('\n'); // Using \n for simplicity, but let's see.
  final encoded = cp949.encode(finalContent);
  await file.writeAsBytes(encoded);
  
  print('Successfully fixed and re-written locations.csv (Line count: ${lines.length})');
}
